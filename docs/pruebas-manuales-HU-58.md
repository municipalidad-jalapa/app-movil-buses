...
1. Abrir `http://localhost:5173/`, tocar una parada y "Estoy esperando aquí".
   (Desde el rediseño del sprint 5 la reserva vive en la hoja del mapa; el QR
   `/registro/:id` abre esa misma hoja con la parada elegida.)
2. **Verificar:** en la hoja "Ya avisamos que estás esperando" aparece la tarjeta "Avisos" con el texto *"Te avisamos cuando el
...
3. Soltar la reserva, recargar la página y volver a reservar.
...
> El endpoint ya existe (rama de integracion del sprint 5) y exige la cabecera
> `X-Dispositivo-Id`: la respuesta **sí** se persiste en el servidor. La bandera
> `VITE_SIMULAR_ABORDAJE` se retiró.
>
> Tras la respuesta, "Listo" cierra el mensaje y la hoja vuelve a pedir parada.
...
- **iOS.** Safari solo entrega avisos si la aplicación está guardada en la
  ...
  de alcance.
- **El abordaje del lado del conductor.** Aquí solo se prueba la respuesta del
  pasajero.
