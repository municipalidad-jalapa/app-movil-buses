 ...
 *
 * <p>Los colores, grosores y formas son los del mock `design/MapaJalapa.dc.html`;
 * lo unico que cambia respecto a el es que el fondo son tiles de verdad en lugar
 * de un dibujo propio.
 */
...
export function MapaOpenStreetMap({
  ...
  useEffect(() => {
    ...
    try {
      instancia = new MapaLibre({
        ...
        zoom: 14,
        // La atribucion va siempre visible y no se puede plegar: es requisito de
        // licencia, no un detalle visual (DESIGN.md seccion 8).
  ...
  useEffect(() => {
    ...
    instancia.once('styledata', () => pintarRuta(instancia, ruta, paradaTuyaId, popupPuesto));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [oscuro]);
  ...
  useEffect(() => {
    ...
    pintarRuta(instancia, ruta, paradaTuyaId, popupPuesto);
  ...
  useEffect(() => {
    ...
    if (!marcadorBus.current) {
      marcadorBus.current = new Marker({ element: elementoDelBus() })
        .setLngLat(donde)
        .addTo(instancia);
      return;
    }
    // Mover el marcador existente, no crear otro: MapLibre interpola el
    // desplazamiento y el bus se desliza en vez de saltar (DESIGN.md seccion 8).
    marcadorBus.current.setLngLat(donde);
  }, [posicionBus, listo]);

  return <div ref={contenedor} className="mapa-jalapa__capa" aria-label="Mapa de Jalapa con la ruta del bus" />;
}
...
function pintarRuta(
  ...
  const paradas: FeatureCollection = {
    ...
    features: ruta.paradas.map((p) => ({
      type: 'Feature',
      properties: { nombre: p.nombre, tuya: p.id === paradaTuyaId },
      geometry: { type: 'Point', coordinates: [p.longitud, p.latitud] },
    })),
  };

  ponerFuente(instancia, FUENTE_RUTA, linea);
  ponerFuente(instancia, FUENTE_PARADAS, paradas);

  // Contorno claro debajo del trazo: lo despega del fondo del mapa.
  ponerCapa(instancia, {
    id: 'ruta-contorno',
    type: 'line',
    source: FUENTE_RUTA,
    layout: { 'line-join': 'round', 'line-cap': 'round' },
    paint: { 'line-color': CONTORNO, 'line-width': 16 },
  });
  ponerCapa(instancia, {
    id: 'ruta-trazo',
    type: 'line',
    source: FUENTE_RUTA,
    layout: { 'line-join': 'round', 'line-cap': 'round' },
    paint: { 'line-color': VERDE_RUTA, 'line-width': 8 },
  });
  ...
  // Tu parada: mas grande y amarilla. Color + forma + tamano, nunca solo color.
  ponerCapa(instancia, {
    id: 'parada-tuya',
    type: 'circle',
    source: FUENTE_PARADAS,
  ...
  if (!popupPuesto.current) {
    ...
    instancia.on('mouseenter', 'paradas', () => (instancia.getCanvas().style.cursor = 'pointer'));
    instancia.on('mouseleave', 'paradas', () => (instancia.getCanvas().style.cursor = ''));
    popupPuesto.current = true;
  }
}

function ponerFuente(instancia: MapaLibre, id: string, datos: Feature | FeatureCollection) {
  const existente = instancia.getSource(id) as GeoJSONSource | undefined;
  if (existente) existente.setData(datos as GeoJsonDato);
  else instancia.addSource(id, { type: 'geojson', data: datos as GeoJsonDato });
}
...
/** El marcador del bus, con la misma forma del mock. */
function elementoDelBus(): HTMLElement {
  const nodo = document.createElement('div');
  nodo.className = 'marcador-bus';
  nodo.setAttribute('aria-label', 'Dónde va el bus');
  nodo.innerHTML = `
    <svg width="60" height="70" viewBox="-30 -46 60 70" aria-hidden="true">
      <path d="M0 -40 L11 -25 L-11 -25 Z" fill="${VERDE_RUTA}" stroke="${CONTORNO}" stroke-width="3"/>
      <circle r="23" fill="${VERDE_RUTA}" stroke="${CONTORNO}" stroke-width="4"/>
      <g transform="translate(-11,-11)">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
             stroke="${CONTORNO}" stroke-width="2.2" stroke-linecap="round">
