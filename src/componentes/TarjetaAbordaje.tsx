...
interface Props {
  ...
  preguntando: boolean;
  /** Cierra el mensaje de resultado y deja elegir parada otra vez. */
  onListo?: () => void;
}

export function TarjetaAbordaje({ preguntando, onListo }: Props) {
...
function Resuelto({ children, onListo }: { children: ReactNode; onListo?: () => void }) {
  return (
    <div className="abordaje">
      <div className="abordaje--resuelto" role="status" aria-live="polite">
        {children}
      </div>
      {onListo && (
        <button type="button" className="hoja-reserva__primario" onClick={onListo}>
          Listo
        </button>
      )}
    </div>
