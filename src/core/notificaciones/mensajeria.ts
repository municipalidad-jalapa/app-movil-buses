 ...
 *
 * La app de Firebase es la misma de la sesion del conductor (`core/firebase.ts`,
 * HU-129): no se inicializa otra. El modulo de mensajeria entra por import
 * dinamico, asi el SDK de avisos solo se descarga cuando se usa.
 */
...
export async function obtenerTokenNotificacion(): Promise<string | null> {
  ...
  const [{ appFirebase }, { getMessaging, getToken, isSupported }] = await Promise.all([
    import('../firebase'),
...
export async function escucharAvisosEnPrimerPlano(
  ...
  const [{ appFirebase }, { getMessaging, onMessage, isSupported }] = await Promise.all([
    import('../firebase'),
    import('firebase/messaging'),
  ]);
