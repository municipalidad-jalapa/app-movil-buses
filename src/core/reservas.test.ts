...
vi.mock('./identidadDispositivo', () => ({
  obtenerIdDispositivo: () => 'dispositivo-prueba',
}));

const conDispositivo = { cabeceras: { 'X-Dispositivo-Id': 'dispositivo-prueba' } };
...
describe('confirmarAbordaje', () => {
  it('manda subio true y devuelve el estado ABORDO', async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/api/v1/reservas/7/abordaje', { subio: true }, conDispositivo);
  });
  ...
  it('manda subio false y devuelve el estado CANCELADA', async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith('/api/v1/reservas/7/abordaje', { subio: false }, conDispositivo);
  });
