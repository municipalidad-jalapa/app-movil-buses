...
import { INTERVALO_ACTUALIZACION_DATOS_MS } from '../../../src/core/frecuenciaActualizacion';
import type { Ruta, RutaEta, ReservasActivasPorParada } from '../../../src/core/tipos';
...
describe('Desarrollo-75 HU-75 - usePanelConductor', () => {
  it('AC2: pide el ETA y las reservas de la ruta activa de la sesion, no de una elegida a mano', async () => {
    mockearApi({});
  ...
  it('AC1: combina ETA y reservas por parada en las filas del panel', async () => {
    mockearApi({});
  ...
  it('Contrato: un 403 (ruta que no es del conductor) llega como error, sin datos enganosos', async () => {
  ...
  it('AC3: vuelve a consultar a la misma cadencia que usa la pantalla del pasajero', async () => {
    mockearApi({});
